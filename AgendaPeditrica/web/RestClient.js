...
// Model for Hijos entity
    models.Hijos = Backbone.Model.extend({
        urlRoot: "http://localhost:49275/Agenda/webresources/ws.database.hijos/",
        idAttribute: 'id',
        defaults: {
            apellido: "",
            nombre: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:49275/Agenda/webresources/ws.database.hijos/';
            }
    ...
    // Collection class for Hijos entities
    models.HijosCollection = Backbone.Collection.extend({
        model: models.Hijos,
        url: "http://localhost:49275/Agenda/webresources/ws.database.hijos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.HijosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-hijos-list-itemis template identifier for item
                        templateName: '#tpl-hijos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.hijos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.hijos,
                    // tpl-hijos-details is template identifier for chosen model element
                    templateName: '#tpl-hijos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Hijos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-hijos-details is a template identifier for chosen model element
                templateName: '#tpl-hijos-details',
